...
WorkerManager::WorkerManager()
{
	...
	//{
	//	cout << "职工编号：" << this->m__EmpArray[i]->m_Id << " "
	//		<< "姓名：" << this->m__EmpArray[i]->m_Name << " "
	//		<< "部门编号：" << this->m__EmpArray[i]->m_DeptId << endl;
	//}
	...
	else
	{
		...
		else if (select == 2)//按照姓名查找
		{
			...
			for (int i = 0; i < m_EmpNum; i++)
			{
				...
				{
					cout << "查找成功，职工编号为：" << m__EmpArray[i]->m_Id << "号的信息如下：" << endl;
					flag = true;
					this->m__EmpArray[i]->showInfo();
				}
			}
...
void WorkerManager::Sort_Emp()
{
	...
	else
	{
		...
		this->Show_Emp();//展示所有的职工   含有按任意键清屏程序
	}
}
